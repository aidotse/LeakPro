...
def run_strategy(
    ...
    if strategy_name == "Dimitrov (prior)":
    ...
    elif strategy_name == "Dimitrov (no prior)":
        # Separate images per epoch but NO order-invariant prior
        config.epoch_handling_strategy = "multi_epoch_separate"
        config.fedavg_lambda_inv = 0.0  # Disable prior
        
    elif strategy_name == "Shared (Geiping)":
        # Reuse same images across epochs (assumes same batch order)
        config.epoch_handling_strategy = "repeated_same"
        config.fedavg_lambda_inv = 0.0  # No prior needed with shared variables
        
    elif strategy_name == "FedSGD-Epoch":
        # Single batch per epoch (Bc=1), so no regularization needed
        # Still simulates multiple epochs but with batch_size=1
        config.epoch_handling_strategy = "multi_epoch_separate"
        config.use_client_batch_size = False  # Override client's batch size
        config.training_simulator_batch_size = batch_size
        config.fedavg_lambda_inv = 0.0  # No regularization needed
        
    elif strategy_name == "FedSGD (Geng)":
        # Treat as single-epoch averaged gradient attack
        config.use_client_epochs = False  # Don't use client's epochs
        config.training_simulator_epochs = 1  # No multi-epoch simulation
        config.epoch_handling_strategy = "repeated_same"
        config.fedavg_lambda_inv = 0.0
...
def main():
    ...
    print("=" * 80)
    print()
    print("Comparing 5 different epoch handling strategies:")
    print("1. Dimitrov (prior) - Full Dimitrov with order-invariant prior")
    print("2. Dimitrov (no prior) - Separate epochs without prior")
    print("3. Shared (Geiping) - Shared variables across epochs")
    print("4. FedSGD-Epoch - Single batch per epoch")
    print("5. FedSGD (Geng) - Single averaged gradient")
    print()
    ...
    for name in ["Dimitrov (prior)", "Dimitrov (no prior)", "Shared (Geiping)", "FedSGD-Epoch", "FedSGD (Geng)"]:
        print(f"\n{'='*80}")
    ...
    visualize_multiple_attacks(
        ...
        data_mean=data_mean,
        data_std=data_std,
        file="multi_epoch_comparison.png",
    )
