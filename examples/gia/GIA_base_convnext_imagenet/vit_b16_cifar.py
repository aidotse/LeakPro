...
def vit_b_16_cifar(
    *, weights: Optional[ViT_B_16_Weights] = None, progress: bool = True, **kwargs: Any
    ...
    """
    CIFAR10-friendly ViT-B variant:
      - image_size=32
      - patch_size=4  (so 32/4=8 -> 8x8=64 patch tokens + 1 cls token)
      - num_classes=10

    Note: typically trained from scratch (weights=None). If you load ImageNet weights,
    you must interpolate positional embeddings (see interpolate_embeddings()).
    """
    ...
    # If you pass pretrained ImageNet weights, keep torchvision behavior consistent:
    # - image_size forced to weights.meta["min_size"][0] (224)
    # - patch_size forced to 16 (because these weights are for ViT-B/16)
    if weights is not None:
        _ovewrite_named_param(kwargs, "num_classes", len(weights.meta["categories"]))
        _ovewrite_named_param(kwargs, "image_size", weights.meta["min_size"][0])
        patch_size = 16
    else:
        patch_size = kwargs.pop("patch_size", 4)  # allow override (4 default; 8 is also reasonable)
