...
class MaxVit(nn.Module):
    ...
    def __init__(
        ...
        block_input_sizes = []
        block_input_shape = _get_conv_output_shape(input_size, 3, stem_stride, 1)  # after stem
        for _ in range(len(block_channels)):
            block_input_shape = _get_conv_output_shape(block_input_shape, 3, 2, 1)  # block downsample
            block_input_sizes.append(block_input_shape)
        ...
        input_size = _get_conv_output_shape(input_size, kernel_size=3, stride=stem_stride, padding=1)
...
def maxvit_t_cifar(*, weights: Optional[MaxVit_T_Weights] = None, progress: bool = True, **kwargs: Any) -> MaxVit:
    """
    ...
    partition_size = kwargs.pop("partition_size", 2)
