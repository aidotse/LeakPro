...
Reads the `[name] ... DONE ssim=... psnr=... best_loss=...` line the runner prints, plus the config
echoed on the first line, so the table stays in sync with whatever sweeps have been run.
"""
import argparse
import ast
import glob
...
DONE = re.compile(r"\[(?P<name>[^\]]+)\] DONE ssim=(?P<ssim>[\d.]+) psnr=(?P<psnr>[-\d.]+) "
                  r"best_loss=(?P<loss>[\d.]+) time=(?P<time>\d+)s")
...
for path in sorted(glob.glob(args.glob)):
    ...
    if best:
        ...
        rows.append({"run": path.split("/")[-1].removesuffix(".log"), "model": "optuna search",
                     ...
                     "lr": None, "loss": None, "ssim": float(best.group("value")), "time": None,
...
cols = ["run", "model", "objects", "img", "iters", "attack", "tv", "lr", "loss", "ssim"]
print("| " + " | ".join(cols) + " |")
print("|" + "|".join("---" for _ in cols) + "|")
for row in sorted(rows, key=lambda r: -r["ssim"]):
    cells = ["" if row.get(c) is None else
             (f"{row[c]:.4f}" if c in {"loss", "ssim"} else str(row.get(c))) for c in cols]
    print("| " + " | ".join(cells) + " |")
