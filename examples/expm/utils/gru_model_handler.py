...
class GRUD(nn.Module):
    def __init__(self, input_size, cell_size, hidden_size, X_mean, batch_size = 0,
        ...
        #TODO: this part differs from gitcode
...
def dpsgd_gru_trained_model_and_metadata(model,
    ...
    scheduler = ReduceLROnPlateau(priv_opt, mode="min",  patience = patience_lr)
    ...
    for epoch in tqdm(range(epochs), desc="Training Progress"):
        ...
        binary_predictions = (prediction > 0).float().cpu().numpy()
        ...
        if output_last:
            test_loss = criterion_BCE(prediction_test, labels_test)
        else:
        ...
        # Convert predictions to class indices
        binary_predictions_test = (prediction_test > 0).float().cpu().numpy()
    ...
    meta_data = {}
    meta_data["train_indices"] = priv_train_dataloader.dataset.indices
