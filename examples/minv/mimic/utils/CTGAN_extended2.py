...
class ContinuousSpec:
    ...
    n_components: int = 1
    mu: torch.Tensor = None      # shape [n_components]
    sigma: torch.Tensor = None   # shape [n_components]

@dataclass
class DiscreteSpec:
    kind: Literal["discrete"] = "discrete"
    n_categories: int = 1
    # if you want to map to integer codes later, keep a list of labels; not needed for differentiable path
...
class CTGANDecoder(nn.Module):
    ...
    def __init__(self, specs: List[ColumnSpec], hard_discrete: bool=False, gumbel_tau: float=0.5):
        ...
        # Register mus/sigmas as buffers so they move with .to(device)
        for name, spec in self.specs:
            if isinstance(spec, ContinuousSpec):
                self.register_buffer(f"{name}__mu", spec.mu.view(-1))
    ...
    def forward(self, h: torch.Tensor) -> torch.Tensor:
        ...
        for name, spec in self.specs:
            if isinstance(spec, ContinuousSpec):
                # slice alpha and beta
                alpha = h[:, idx]                  # [B]
                beta  = h[:, idx+1: idx+1+spec.n_components]  # [B, K]
                idx += 1 + spec.n_components

                p = F.softmax(beta, dim=-1)        # [B, K]
                # clamp alpha very slightly to be safe
                alpha = alpha.clamp(-0.999, 0.999) # [B]

                mu    = getattr(self, f"{name}__mu")    # [K]
                sigma = getattr(self, f"{name}__sigma")  # [K]

                # x_hat = sum_j p_j * (mu_j + 4*sigma_j*alpha)
                # broadcast alpha to [B, K]
                x_comp = mu + 4.0 * sigma * alpha.unsqueeze(1)  # [B, K]
                x_hat  = (p * x_comp).sum(dim=1, keepdim=True)  # [B, 1]
                outs.append(x_hat)

            else:  # DiscreteSpec
                d = h[:, idx: idx+spec.n_categories]           # [B, C]
                idx += spec.n_categories

                if self.hard_discrete:
                    # Straight-through Gumbel-Softmax (categorical sample in fwd, gradient through soft)
                    y_soft = F.gumbel_softmax(d, tau=self.gumbel_tau, hard=True, dim=-1)  # [B, C]
                    outs.append(y_soft)  # remains differentiable via STE
                else:
                    # Expected one-hot (just use softmax probs)
                    y_prob = F.softmax(d, dim=-1)               # [B, C]
                    outs.append(y_prob)
