...
class CelebA_InputHandler(AbstractInputHandler):  # noqa: N801
    """Class to handle the user input for the CelebA dataset for plgmi attack."""

    def __init__(self, configs: dict) -> None:
        super().__init__(configs=configs)
        print("Configurations:", configs)  # noqa: T201
    ...
    def train_gan(self,  # noqa: D417, PLR0915
        ...
        for i in range(n_iter):
            ...
            for j in range(n_dis):
                ...
                with torch.no_grad():
                    count += fake.shape[0]
                    T_logits = target_model(fake)  # noqa: N806
                    T_preds = T_logits.max(1, keepdim=True)[1]  # noqa: N806
                    target_correct += T_preds.eq(fake_labels.view_as(T_preds)).sum().item()
                    cumulative_target_acc += round(target_correct / count, 4)

            if i % log_interval == 0:
                print(  # noqa: T201
                        "iteration: {:05d}/{:05d}, loss gen: {:05f}, loss dis {:05f}, inv loss {:05f}, target acc {:04f}, time {}".format(  # noqa: E501
