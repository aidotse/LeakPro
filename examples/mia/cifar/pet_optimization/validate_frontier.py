...
def make_revalidate_fn(splits: dict, recipe: PETRecipe, n_refs: int, device: str,
    ...
    def revalidate_fn(config: dict) -> AttackScores:
        ...
        for _ in range(n_refs):
            ...
            ref_phi_m += confidence_logits(ref, x[members], y[members], device, kind) / n_refs
            ref_phi_n += confidence_logits(ref, x[nonmembers], y[nonmembers], device, kind) / n_refs
            del ref
            if device.startswith("cuda"):
                torch.cuda.empty_cache()

        scores = AttackScores(
            member_scores=confidence_logits(target, x[members], y[members], device, kind) - ref_phi_m,
            nonmember_scores=confidence_logits(target, x[nonmembers], y[nonmembers], device, kind) - ref_phi_n,
        )
        del target
        if device.startswith("cuda"):
            torch.cuda.empty_cache()
