...
def encode_categorical(
    ...
    """Encode dataframes with categorical values using both dataframes for labels."""
...
def scale_numerical(df1: pd.DataFrame, df2: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
...
def mixed_types_transform(
    ...
    Categorical columns are label encoded, using both datasets for labeling.
    ...
    cat_cols: list[str].
        Names of the categorical columns to be processed.
    ...
    if len(num_cols) > 0:
        df1_num, df2_num = scale_numerical(df1[num_cols], df2[num_cols])
    df1_cat, df2_cat = pd.DataFrame(), pd.DataFrame()
    if len(cat_cols) > 0:
        df1_cat, df2_cat = encode_categorical(df1[cat_cols], df2[cat_cols])
