...
def multivariate_singling_out_queries(df: pd.DataFrame, n_queries: int, n_cols: int, max_attempts: Optional[int]) -> List[str]:
...
def main_singling_out_attack(
    ori: pd.DataFrame, syn: pd.DataFrame, n_attacks: int, n_cols: int, max_attempts: Optional[int]
    ...
    if n_cols == 1:
        ...
        queries = multivariate_singling_out_queries(df=syn, n_queries=n_attacks, n_cols=n_cols, max_attempts=max_attempts)
    # Warning message
...
class SinglingOutEvaluator(BaseModel):
    ...
    def evaluate(self: Self) -> EvaluationResults:
        ...
        self.main_queries = main_singling_out_attack(
            ori=self.ori, syn=self.syn, n_attacks=self.n_attacks, n_cols=self.n_cols, max_attempts=self.max_attempts
        )
