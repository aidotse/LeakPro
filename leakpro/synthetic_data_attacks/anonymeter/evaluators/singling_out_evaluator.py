...
def univariate_singling_out_queries(*, df: pd.DataFrame, ori: pd.DataFrame, n_queries: int) -> List[str]:
    ...
    logger.info(
        f"[SinglingOut] Strategy selection: rare={rare_evaluator.count}/{len(rare_max)} ({rare_proportion:.2%}), boundary={boundary_evaluator.count}/{len(boundary_max)} ({boundary_proportion:.2%})"
    )
