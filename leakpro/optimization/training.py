...
from leakpro.optimization.objectives import AttackScores
...
@torch.no_grad()
def confidence_logits(  # noqa: PLR0913
    model: Module,
    x: torch.Tensor,
    y: torch.Tensor,
    device: str,
    ...
    for i in range(0, len(x), batch):
        ...
        if output_kind == "binary_probs":
            ...
            p_true = probs.gather(1, y[i:i + batch].reshape(-1, 1).long()).squeeze(1)
...
def build_campaign_fns(  # noqa: PLR0913
    ...
    def attack_fn(model: Module, config: dict) -> AttackScores:
        ...
        for _ in range(n_refs):
            ...
            ref_phi_m += confidence_logits(ref, x[members], y[members], device, recipe.output_kind) / n_refs
            ref_phi_n += confidence_logits(ref, x[nonmembers], y[nonmembers], device, recipe.output_kind) / n_refs
            del ref
            _free_cuda(device)

        return AttackScores(
            member_scores=confidence_logits(model, x[members], y[members], device, recipe.output_kind) - ref_phi_m,
            nonmember_scores=(
                confidence_logits(model, x[nonmembers], y[nonmembers], device, recipe.output_kind) - ref_phi_n
            ),
        )
